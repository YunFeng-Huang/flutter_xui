...
class DateUtil {
  ...
  /// is yesterday by dateTime.
  /// 是否是前天.
  static bool isBeforeYesterday(DateTime dateTime, DateTime locDateTime) {
    if (yearIsEqual(dateTime, locDateTime)) {
      int spDay = getDayOfYear(locDateTime) - getDayOfYear(dateTime);
      return spDay == 2;
    } else {
      return ((locDateTime.year - dateTime.year == 2) &&
          dateTime.month == 12 &&
          locDateTime.month == 1 &&
          dateTime.day == 31 &&
          locDateTime.day == 1);
  ...
  /// is yesterday by millis.
  /// 是否是前天.
  static bool isBeforeYesterdayByMs(int ms, int locMs) {
    return isYesterday(DateTime.fromMillisecondsSinceEpoch(ms),
        DateTime.fromMillisecondsSinceEpoch(locMs));
  }
